...
public class Part2b {
	...
	private static void check_emergency() {
		...
		if (random.nextDouble() < Parameters.p_emergency) {
			...
			// -----
			int randQ = random.nextInt(4);
			int randPatient = random.nextInt(Q[randQ].size);

			Patient p = Q[randQ].get(randPatient);

			Q[randQ].dequeue(randPatient);
	...
	private static void check_promotion() {
		...
		// -----
		for(int y = 0; y < 2; y++){
			for(int x = 0; x < Q[y].size; x++){
				if(Q[y].get(x).wait_time(Parameters.l12_escalation_delay) > Parameters.l12_escalation_delay){
					Patient p = Q[y].dequeue(x);
					Q[y+1].push(p);
				}
			}
		}
	...
	private static void check_walkout() {
		...
		if (random.nextDouble() < Parameters.p_walkout) {
			...
			// -----
            int randPatient = random.nextInt(Q[0].size);
            Q[0].dequeue(randPatient);
